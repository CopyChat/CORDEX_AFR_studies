...
dailystd_CORDEX_remap=np.array([ctang.read_lonlatmap_netcdf(VAR,\
        Data+VAR+'_AFR-44_'+RCM_Model[i]+filefix_Remap[1]) for i in range(N_model)])
dailystd_OBS=np.array(ctang.read_lonlatmap_netcdf(OBSvar, OBS_Dir+OBSfile[1]))
...
Ensmean_dailystd_CORDEX_remap=np.mean(dailystd_CORDEX_remap,axis=0)*100/Ensmean_timmean_CORDEX_remap
Ensmean_monstd_CORDEX_remap=np.mean(monstd_CORDEX_remap,axis=0)*100/Ensmean_timmean_CORDEX_remap
Ensmean_annualstd_CORDEX_remap=np.mean(annualstd_CORDEX_remap,axis=0)*100/Ensmean_timmean_CORDEX_remap
...
Ensmean_dailystd_Bias=(Ensmean_dailystd_CORDEX_remap-dailystd_OBS)
Ensmean_monstd_Bias=(Ensmean_monstd_CORDEX_remap-monstd_OBS)
Ensmean_annualstd_Bias=(Ensmean_annualstd_CORDEX_remap-annualstd_OBS)
...
Samples1 = np.array([[np.nanstd(m,ddof=1)/RefStd[1], \
        ...
        [np.ma.array(dailystd_CORDEX_remap[i,:,:].flatten(), \
            mask=np.isnan(dailystd_CORDEX_remap[i,:,:].flatten())) \
            for i in range(N_model)]])

Samples2 = np.array([[np.nanstd(m,ddof=1)/RefStd[2], \
        np.ma.corrcoef(monstd_OBS.flatten(),m,allow_masked='Ture')[0,1]] for m in \
        [np.ma.array(monstd_CORDEX_remap[i,:,:].flatten(), \
            mask=np.isnan(monstd_CORDEX_remap[i,:,:].flatten())) \
            for i in range(N_model)]])

Samples3 = np.array([[np.nanstd(m,ddof=1)/RefStd[3], \
        np.ma.corrcoef(annualstd_OBS.flatten(),m,allow_masked='Ture')[0,1]] for m in \
        [np.ma.array(annualstd_CORDEX_remap[i,:,:].flatten(), \
            mask=np.isnan(annualstd_CORDEX_remap[i,:,:].flatten())) \
            for i in range(N_model)]])
...
plt.savefig('evaluation.rsds.era40.png')
plt.show()
