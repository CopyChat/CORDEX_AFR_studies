        ...
        for m in range(N_model):
            ...
            for lat in range(t_value.shape[2]):
                for lon in range(t_value.shape[3]):
                    if abs(stats.ttest_1samp( t_value[v,:,lat,lon],0)[0]) > T:
                        count+=1
                    else:
                        t_value[v,m,lat,lon]=np.NaN
    print "get good point: ",str(count*100/N_var/N_model/t_value.shape[2]/t_value[3])
    return t_value
#--------------------------------------------------- 
...
plt.savefig('map.meanchange.rsds.tas.vws.png')
plt.show()
