...
#=================================================== plot every model, RCMs and GCMs
    ...
    # for m in range(len(GCM)):
        # if m in [ 1,2,4]: # interactive aerosols
            # axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='blue',label='interactive aerosols' if m == 2 else "")
        # if m in [ 3,5]: # Semi-interactive aerosols
            # axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='green',label='Semi-interactive aerosols' if m == 3 else "")
        # if m in [ 0,8]: # prescribed aerosols
            # axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='red',label='prescribed aerosols' if m == 0 else "")


# wether model has First/second aerosol-cloud indirect effect
    for m in range(len(GCM)):
        if m in [ 4,7,9]: # Y/Y
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='blue',label='Y/Y' if m == 4 else "")
        if m in [ 0,1,2,5,6]: # Y/N
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='green',label='Y/N' if m == 0 else "")
        if m in [ 3,8]: # N/N
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='red',label='N/N' if m == 3 else "")
...
#=================================================== plot ensstd of anomalies
    ...
    axes[k].legend(loc='upper left', shadow=False,prop={'size':6})
...
# plt.savefig('rsds.annual_series.gcm-rcm.by_aerosol.eps', format='eps')
plt.savefig('rsds.annual_series.gcm-rcm.by_aerosol.png')
plt.show()
quit()
