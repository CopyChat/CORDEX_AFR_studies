...
filefix=('.hist.day.1970-1999.SA.timmean.nc' ,\
...
monstd_OBS = np.array(GEBA.mon_std)
annualstd_OBS = np.array(GEBA.year_std)

# Ensmean of Bias

print Ensmean_timmean_CORDEX.shape
...
for i in range(GEBA.lon.count()):
    Ensmean_timmean_Bias[i]=Ensmean_timmean_CORDEX[idx[1,i],idx[0,i]] - float(GEBA.yearmean[i])
    Ensmean_monstd_Bias[i]=Ensmean_monstd_CORDEX[idx[1,i],idx[0,i]] - float(GEBA.mon_std[i])
    Ensmean_annualstd_Bias[i]=Ensmean_annualstd_CORDEX[idx[1,i],idx[0,i]] - float(GEBA.year_std[i])
...
print Ensmean_timmean_Bias
print Ensmean_monstd_Bias
print Ensmean_annualstd_Bias
#=================================================== input
## Reference dataset
...
def PlotScatter(x,y,c,m,k,axx,vmin,vmax):
    ...
    print vmin,vmax,'kkkkkkkkkkkkkkk'

    sc=plt.scatter(x, y, c=c, cmap=cmap,norm=norm,edgecolors='b',zorder=2,vmin=vmin, vmax=vmax,\
            s=35)
