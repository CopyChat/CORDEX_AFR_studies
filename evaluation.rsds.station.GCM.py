...
lons,lats=ctang.read_lonlat_netcdf_1D(\
        Data+VAR+GCM_Model[1]+'.hist.1970-1999.SA.timmean.remap.nc')
...
filefix=(\
        ...
        '_historical-rcp85_r1i1p1.1970-1999.SA.yearmean.detrended.masknoooon.timstd.remap.nc',\
        '_historical-rcp85_r1i1p1.1970-1999.SA.monmean.detrended.maskannual.timstd.remap.nc',\
        '_historical-rcp85_r1i1p1.1970-1999.SA.yearmean.detrended.masknoooon.timstd.remap.nc')
...
timmean_CMIP5=np.array([ctang.read_lonlatmap_netcdf(VAR,\
        Data+VAR+GCM_Model[i]+filefix[0])\
        for i in range(N_model)])

# Read Ensmean of daily std for CMIP5 & OBS
#dailystd_CMIP5=np.array([ctang.read_lonlatmap_netcdf(VAR,\
        #Data+VAR+GCM_Model[i]+filefix[1]) for i in range(N_model)])

# Read Ensmean of monthly std for CMIP5 & OBS
monstd_CMIP5=np.array([ctang.read_lonlatmap_netcdf(VAR,\
        Data+VAR+GCM_Model[i]+filefix[2]) for i in range(N_model)])

# Read Ensmean of annual std for CMIP5 & OBS
annualstd_CMIP5=np.array([ctang.read_lonlatmap_netcdf(VAR,\
        Data+VAR+GCM_Model[i]+filefix[3]) for i in range(N_model)])

#=================================================== cal
# remove the missing points
#timmean_CMIP5_remap[timmean_CMIP5_remap > 100055] = np.nan
#dailystd_CMIP5_remap[dailystd_CMIP5_remap > 100000] = np.nan
#monstd_CMIP5_remap[monstd_CMIP5_remap > 100000] = np.nan
#annualstd_CMIP5_remap[annualstd_CMIP5_remap > 100000] = np.nan

# Ensmean of CMIP5 
Ensmean_timmean_CMIP5=np.mean(timmean_CMIP5,axis=0)
#Ensmean_dailystd_CMIP5=np.mean(dailystd_CMIP5,axis=0)*100/Ensmean_timmean_CMIP5
Ensmean_monstd_CMIP5=np.mean(monstd_CMIP5,axis=0)*100/Ensmean_timmean_CMIP5
Ensmean_annualstd_CMIP5=np.mean(annualstd_CMIP5,axis=0)*100/Ensmean_timmean_CMIP5

# CMIP5 over the satation
timmean_CMIP5_station=np.zeros((N_model,GEBA.lon.count()))
monstd_CMIP5_station=np.zeros((N_model,GEBA.lon.count()))
annualstd_CMIP5_station=np.zeros((N_model,GEBA.lon.count()))
for n in range(N_model):
    for i in range(GEBA.lon.count()):
        timmean_CMIP5_station[n,i]=timmean_CMIP5[n,idx[1,i],idx[0,i]]
        monstd_CMIP5_station[n,i]=monstd_CMIP5[n,idx[1,i],idx[0,i]]
        annualstd_CMIP5_station[n,i]=annualstd_CMIP5[n,idx[1,i],idx[0,i]]

# for OBS
#dailystd_OBS = dailystd_OBS*100/Ensmean_timmean_CMIP5_remap
timmean_OBS= np.array(GEBA.yearmean)
monstd_OBS = np.array(GEBA.mon_std)*100/np.mean(timmean_CMIP5_station,axis=0)
annualstd_OBS = np.array(GEBA.year_std)*100/np.mean(timmean_CMIP5_station,axis=0)
...
for i in range(GEBA.lon.count()):
    Ensmean_timmean_Bias[i]=Ensmean_timmean_CMIP5[idx[1,i],idx[0,i]] - timmean_OBS[i]
    Ensmean_monstd_Bias[i]=Ensmean_monstd_CMIP5[idx[1,i],idx[0,i]] - monstd_OBS[i]
    Ensmean_annualstd_Bias[i]=Ensmean_annualstd_CMIP5[idx[1,i],idx[0,i]] - annualstd_OBS[i]
...
Samples0 = np.array([[np.nanstd(m,ddof=1)/RefStd[0], \
        ...
        for m in [np.ma.array(timmean_CMIP5_station[i,:], \
            mask=np.isnan(timmean_CMIP5_station[i,:])) \
            for i in range(N_model)]])

# time variability are needed to be normalised by Ensmean_timmean_CMIP5_remap
Samples1 = np.array([[np.nanstd(m,ddof=1)/RefStd[1], \
        np.ma.corrcoef(monstd_OBS,m,allow_masked='Ture')[0,1]] for m in \
        [np.ma.array(monstd_CMIP5_station[i,:], \
            mask=np.isnan(monstd_CMIP5_station[i,:])) \
            for i in range(N_model)]])

Samples2 = np.array([[np.nanstd(m,ddof=1)/RefStd[2], \
        np.ma.corrcoef(annualstd_OBS.flatten(),m,allow_masked='Ture')[0,1]] for m in \
        [np.ma.array(annualstd_CMIP5_station[i,:].flatten(), \
            mask=np.isnan(annualstd_CMIP5_station[i,:].flatten())) \
            for i in range(N_model)]])
...
plt.savefig('evaluation.rsds.station.GCM.png')
plt.show()
