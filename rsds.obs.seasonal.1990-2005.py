...
def PlotMap(array2D,lons,lats,m,k,axx,vmin,vmax,cmap):
    ...
    map=Basemap(projection='cyl',llcrnrlat=-39.9,urcrnrlat=-0.1,\
...
for S in range(2):
    ...
    Reference='SISmm.SARAH-2.1990-2005.SA.'+str(Season[S])+'.timmean.nc'

    OBSfile=(\
        'srb_rel3.0_shortwave_monthly_utc.1990-2005.SA.'+str(Season[S])+'.timmean.nc',\
        'ERA_In.ssrd.mon.mean.1990-2005.SA.'+str(Season[S])+'.timmean.nc',\
        'cfsr.rsds.mon.1990.2005.SA.'+str(Season[S])+'.timmean.nc',\
        )

    Ref_remap=(\
        'SISmm.SARAH-2.1990-2005.SA.'+str(Season[S])+'.timmean.remap.srb.nc',\
        'SISmm.SARAH-2.1990-2005.SA.'+str(Season[S])+'.timmean.remap.era_in.nc',\
        'SISmm.SARAH-2.1990-2005.SA.'+str(Season[S])+'.timmean.remap.cfsr.nc',\
        )
