...
#=================================================== plot every GCMs
    ...
    # for m in range(len(GCM)):
        # if m in [ 1,3]: # interactive aerosols
            # axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='blue',label='interactive aerosols' if m == 1 else "")
        # if m in [ 2,4]: # Semi-interactive aerosols
            # axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='green',label='Semi-interactive aerosols' if m == 2 else "")
        # if m in [ 0,6]: # prescribed aerosols
...
# wether model has First/second aerosol-cloud indirect effect
    for m in range(len(GCM)):
        if m in [ 3,5,7]: # Y/Y
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='blue',label='Y/Y' if m == 3 else "")
        if m in [ 0,1,4,]: # Y/N
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='green',label='Y/N' if m == 0 else "")
        if m in [ 2,6]: # N/N
            axes[k].plot(TIME[15:116], Anomaly2[m,k,:], '--',linewidth=1.5,color='red',label='N/N' if m == 2 else "")
...
#=================================================== plot ensstd of anomalies
    ...
    axes[k].legend(loc='lower left', shadow=False,prop={'size':6})
