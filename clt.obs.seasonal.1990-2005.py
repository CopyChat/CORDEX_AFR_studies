...
def PlotMap(array2D,lons,lats,m,k,axx,vmin,vmax,cmap,MBE,RMSE,COR):
    ...
    if(k>0):
        axx.text(0.95, 0.25,'r='+str("%.2f" % COR), ha='right',va='center', transform=axx.transAxes)
        axx.text(0.95, 0.15,'MBE='+str("%.2f" % MBE), ha='right',va='center', transform=axx.transAxes)
        axx.text(0.95, 0.05,'RMSE='+str("%.2f" % RMSE), ha='right',va='center', transform=axx.transAxes)
...
for S in range(2):
    ...
    Reference='CFCmm.CM_SAF.CLARA-A2.1990-2005.GL.SA.'+str(Season[S])+'.timmean.nc'

    OBSfile=(\
        'cltisccp.monmean.1990-2005.SA.'+str(Season[S])+'.timmean.nc',\
        'ERA_In.clt.mon.mean.1990-2005.SA.'+str(Season[S])+'.timmean.nc',\
        'cfsr.clt.mon.1990.2005.SA.'+str(Season[S])+'.timmean.nc',\
        )

    Ref_remap=(\
        'CFCmm.CM_SAF.CLARA-A2.1990-2005.GL.SA.'+str(Season[S])+'.timmean.remap.isccp.nc', \
        'CFCmm.CM_SAF.CLARA-A2.1990-2005.GL.SA.'+str(Season[S])+'.timmean.remap.era_in.nc', \
        'CFCmm.CM_SAF.CLARA-A2.1990-2005.GL.SA.'+str(Season[S])+'.timmean.remap.cfsr.nc', \
        )
...
# Read Ref as CM_saf
    ...
    print S,k,"-----------"
    PlotMap(Ref_OBS,lons,lats,S,k,axx,0,100,cmap,MBE,RMSE,COR)
    ...
    for i in range(N_obs-1):
        ...
        ## remove np.nan:
        Ref_OBS_remap_1d=Ref_OBS_remap.ravel()
        Ref_OBS_nonnan=np.array([value for value in Ref_OBS_remap_1d if not math.isnan(value)])

        # Mean Bias:
        MBE=np.nanmean(OBS.ravel()-Ref_OBS_remap_1d)

        # spatial correlation:
        OBS_nonnan=[OBS.ravel()[i] for i in range(len(Ref_OBS_remap_1d)) if not math.isnan(Ref_OBS_remap_1d[i])]

        COR= np.ma.corrcoef(OBS_nonnan,Ref_OBS_nonnan)[0,1]
        print COR


        # RMSE
        RMSE=np.sqrt(np.nanmean((OBS.ravel()-Ref_OBS_remap_1d)**2))
        ...
        ### if plot the bias
        if BIAS==1:
            PlotMap(OBS-Ref_OBS_remap,lonsOBS,latsOBS,S,k,axx,-50,50,cmap,MBE,RMSE,COR)
        else:
            PlotMap(OBS,lonsOBS,latsOBS,S,k,axx,0,100,cmap,MBE,RMSE,COR)



# plt.suptitle(Title)

OutputImage='rsds.obs.seasonal.1990-2005'
